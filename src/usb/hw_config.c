...
/* Private macro -------------------------------------------------------------*/
#define USE_ARMJISHU_LCD  
/* Private variables ---------------------------------------------------------*/
ErrorStatus HSEStartUpStatus;
__IO uint32_t ADC_ConvertedValueX = 0;
__IO uint32_t ADC_ConvertedValueX_1 = 0;
...
void Set_System(void)
{
  ...
  /* Configure the Key2 button in EXTI mode */
  STM_EVAL_PBInit(Button_KEY, Mode_EXTI);  
  
  /* Configure the Key3/Tamper button in EXTI mode */
...
#ifdef USE_ARMJISHU_LCD  
  /* LCD液晶屏显示 */
  STM3210C_LCD_Init();
  LCD_DisplayStringLine(LCD_LINE_8, "   USB Custom HID   ");
#endif
  
  printf("\n\r  The STM32F10x device is enumerated as HID compliant device,"
         "\n\r       that uses the native PC Host USB-FS-Device HID driver.");
  printf("\n\r  神舟STM32开发板USB人机交互设备HID(Human Interface Device)实验\n\r");

  LCD_DisplayStringLine(LCD_LINE_0, " LED  ADC  KEY2 KEY3");
}
